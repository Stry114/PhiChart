...
class Player:
    ...
    def render(self):
        ...
        for line in self.chart.lineList:
            ...
            for note in line.noteList:
                ...
                if self.timeT > note.time_:
                    ...
                    dyt = dy + (note.speed * note.holdTime * 1.875 / line.bpm) * self.Y
                if self.chart.RPE_Chart:
                    dyt = (note.floorPosT - pos) * self.Y

                if self.enable3D and (not note.ban3D):
                ...
                if self.enable3D and (not note.ban3D) and (dyo > self.boundary or dyo < -self.cmrB * self.Y):
                    continue
        ...
        for line in self.chart.lineList:
            ...
            for note in line.noteList:
                ...
                dx = note.posX * self.X
                dy = note.speed * (note.floorPos - pos) * self.Y
    ...
    def initPlayer(self):
        ...
        # 显示消息
        self.titleMsg("读取谱面中", "PhiChart被玩壞了！這肯定不是PhiChart的問題！肯定不是！")
    ...
    def judgementAutoPlay(self, deltaT):
        ### 键打击判定
        for line in self.chart.lineList:
            for note in line.noteList:
                if note.type_ != 3:
    ...
    def judgementSimulator(self, deltaT):
        # 统计按键情况
        keys = pygame.key.get_pressed()
        newPressedKeysNumber = 0
        allPressedKeysNumber = len(keys)
        for key in keys:

            if key not in self.previousKeysLastFrame:
                newPressedKeysNumber += 1
        self.previousKeysLastFrame = keys
        if newPressedKeysNumber > 0:
            print(keys)
            print(time.time(), newPressedKeysNumber)

        for line in self.chart.lineList:
            for note in line.noteList:
                if note.type_ == 1 or note.type_ == 4:
                    if note.hit:
                        continue
                    elif newPressedKeysNumber <= 0:
                        continue
                    elif abs(note.time_ - self.timeT) / line.bpm * 1.875 > 0.16:
                        continue
                    else:
                        note.hit = True
                        newPressedKeysNumber -= 1
    ...
    def mainloop(self):
        ...
        while running:
            ...
            if key_left_pressed:
                cmr_speed_x = max(-2, cmr_speed_x-0.01)
            elif key_right_pressed:
                cmr_speed_x = min(2, cmr_speed_x+0.01)
            else:
                pass
                cmr_speed_x = 0.95 * cmr_speed_x
            self.cmrX -= cmr_speed_x
            ...
            # 键打击判定
            if self.autoplay:
                self.judgementAutoPlay(deltaT)
            else:
                self.judgementSimulator(deltaT)
    ...
    def header(self):
        ...
        while True:
            ...
            pygame.draw.line(
                self.foreground_layer,
                (254, 255, 169, 255),
                (self.width*(0.5-(ct-t0)**3.6*0.5), self.height*(1-0.5*self.displacementY)),
                (self.width*(0.5+(ct-t0)**3.6*0.5), self.height*(1-0.5*self.displacementY)),
                width=round(self.lineWidth)
            )
            self.screen.blit(self.background_layer, (0, 0))
            self.screen.blit(self.foreground_layer, (0, 0))
            pygame.display.flip()
